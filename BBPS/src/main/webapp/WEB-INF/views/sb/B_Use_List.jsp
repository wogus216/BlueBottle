...
function reloadList(){
	...
	$.ajax({
		...
		success : function(res){
			drawstockuseList(res.list,res.result);
			drawusePaging(res.pb);
		},
...
function drawstockuseList(list,result){
	var html ="";
	
	if(result == 0){ //결과 행이 존재하지 않는 경우
		html += "<tr>";
		html += "<td colspan = \"6\" style = \"text-align: center;\">검색조건에 맞는 데이터가 없거나 사용품목이 존재하지 않습니다.</td>";
		html += "</tr>";	
